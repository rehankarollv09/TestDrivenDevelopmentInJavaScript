describe("Matchers for equality", () => {
  ...
  test("toMatch() matches a string against an RE", () => {
    const message = "Mumbai is best city";

    expect(message).toMatch(/City/i);
    expect(message).toMatch(/mum../i);
    expect(message).not.toMatch(/Go../);
  });
  ...
  test("toMatchObject() matches object properties", () => {
    ...
    expect(carDesired).not.toMatchObject(carOnSale);
  });
});
...
describe("Matchers for collections", () => {
  ...
  test("test for an item in a Set", () => {
    const countries = new Set().add("SA").add("NO").add("SG");
